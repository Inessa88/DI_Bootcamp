...
function playTheGame() {
    ...
    let userNumber = 0;
    let result = confirm("Would you like to play a game?");
    if (result == false) {
...
function compareNumbers(userNumber,computerNumber) {
    ...
    for (let i=1; i<= guesses; i++){
        if (i === 3){
            console.log(alert("Out of chances"));
            break;
...
if (valid) {
    compareNumbers(result[0],result[1])
}
...
// With this condition the function playTheGame will look like this:
...
//         do {
//             userNumber = Number(prompt("Please, enter a number between 0 and 10"));
//         } while (!isOnlyNumbers(userNumber) || userNumber>10 || userNumber <0);
//         computerNumber = Math.floor((Math.random()*10) + 1);
//     }
//     return [userNumber,computerNumber];
// }